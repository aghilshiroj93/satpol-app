    ...
    <div class="p-6 md:p-10 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 overflow-hidden">
            ...
            <div class="slide-in-right">
                <button id="refreshBtn"
                    ...
                    <svg class="w-5 h-5 text-blue-600 group-hover:rotate-180 transition-transform duration-500"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @php
                $stats = [
                    ...
                    [
                        ...
                        'color' => 'red',
                        'icon' =>
                            'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.882 16.5c-.77.833.192 2.5 1.732 2.5z',
                    ],
                    [
                        'value' => $barangDipinjam ?? 0,
            ...
            @foreach ($stats as $index => $stat)
                <div
                    class="stat-card bg-white rounded-2xl p-6 shadow-sm hover-lift scale-in stagger-delay-{{ $index + 1 }} overflow-hidden relative group">
                    <!-- Background effect -->
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-{{ $stat['color'] }}-50/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="mt-2 flex items-center justify-between">
                            ...
                            <div class="stat-card-icon bg-{{ $stat['color'] }}-50 p-4 rounded-xl">
                                <svg class="w-8 h-8 text-{{ $stat['color'] }}-600" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="{{ $stat['icon'] }}" />
                                </svg>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <!-- Perawatan chart (big) - Diubah menjadi per hari -->
            <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm hover-lift rotate-in overflow-hidden">
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                    <div>
                        <h3 class="text-xl font-bold text-gray-800">Aktivitas Perawatan Harian</h3>
                        <p class="text-sm text-gray-500 mt-1">Data 30 hari terakhir</p>
                    </div>

                    <div class="flex items-center gap-3 mt-4 md:mt-0">
                        <!-- Tabs untuk pilihan periode -->
                        <div class="flex bg-gray-100 p-1 rounded-xl">
                            <button
                                class="tab-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 active"
                                data-period="30">
                                30 Hari
                            </button>
                            <button class="tab-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300"
                                data-period="14">
                                14 Hari
                            </button>
                            <button class="tab-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300"
                ...
                <div class="flex flex-wrap gap-4 mb-6">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-gray-600">Total Perawatan</span>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <!-- Chart Container -->
                <div class="mt-4 chart-container relative">
                    <canvas id="perawatanChart" height="180"></canvas>

                    <!-- Stats overlay -->
                    <div class="absolute top-4 right-4 bg-white/80 backdrop-blur-sm rounded-lg p-3 shadow-sm">
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div>
                            <span class="text-xs font-medium text-gray-700">Hari ini: <span id="todayCount">0</span></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-3 gap-4">
                    <div class="text-center p-3 bg-blue-50 rounded-xl">
                        <p class="text-xs text-blue-600 font-medium">Rata-rata/hari</p>
                        <p id="avgPerDay" class="text-lg font-bold text-gray-800">0</p>
                    </div>
                    <div class="text-center p-3 bg-green-50 rounded-xl">
                        <p class="text-xs text-green-600 font-medium">Tertinggi</p>
                        <p id="maxPerDay" class="text-lg font-bold text-gray-800">0</p>
                    </div>
                    <div class="text-center p-3 bg-amber-50 rounded-xl">
                        <p class="text-xs text-amber-600 font-medium">Trend</p>
                        <p id="trendIndicator" class="text-lg font-bold text-gray-800">-</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm hover-lift slide-in-up overflow-hidden">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div
                            class="bg-gradient-to-br from-amber-50 to-amber-100/50 p-4 rounded-xl border border-amber-100">
                        ...
                        <div
                            class="bg-gradient-to-br from-indigo-50 to-indigo-100/50 p-4 rounded-xl border border-indigo-100">
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @forelse($recentPerawatan as $index => $p)
                            <tr class="table-row-animate hover:bg-gray-50 transition-colors duration-200"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-medium text-gray-900">{{ $p->barang->nama_barang ?? '—' }}
                                            </div>
                                            <div class="text-xs text-gray-500">Kode: {{ $p->barang->kode_barang ?? '-' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="relative inline-flex items-center gap-1 px-4 py-1.5 rounded-full text-xs font-semibold
                                    {{ $p->status == 'Selesai'
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                tabButtons.forEach(btn => {
                    btn.addEventListener('click', function() {
                        ...
                        updateChart(parseInt(period));
                    });
                });
            });

            // Data dummy untuk perawatan harian (30 hari terakhir)
            function generateDailyData(days) {
                ...
                for (let i = days - 1; i >= 0; i--) {
                    ...
                    // Generate random data (simulasi)
                    const total = Math.floor(Math.random() * 15) + 3;
                    const selesai = Math.floor(total * (0.6 + Math.random() * 0.3));
                    const dalamProses = total - selesai;
            ...
            let dailyChart = null;
            // Data untuk chart - ambil dari controller
            const perawatanHarian = @json($perawatanHarian ?? []);

            // Initialize chart with 30 days
            // Ganti function initChart dengan ini:
            function initChart(days = 30) {
                ...
                // Gunakan data dari controller (bukan generateDailyData)
                const perawatanData = @json($perawatanHarian ?? []);

                // Filter data berdasarkan periode yang diminta
                const filteredData = perawatanData.slice(-days);

                // Prepare data
                const labels = filteredData.map(d => d.date);
                const totalData = filteredData.map(d => d.total);
                const selesaiData = filteredData.map(d => d.selesai);
                const dalamProsesData = filteredData.map(d => d.dalam_proses);

                // Calculate statistics
                const todayCount = totalData[totalData.length - 1] || 0;
                const avgPerDay = (totalData.reduce((a, b) => a + b, 0) / days).toFixed(1);
                const maxPerDay = Math.max(...totalData);

                // Determine trend
                const lastWeek = totalData.slice(-7);
                const firstWeek = totalData.slice(0, 7);
                const avgLastWeek = lastWeek.reduce((a, b) => a + b, 0) / 7;
                const avgFirstWeek = firstWeek.reduce((a, b) => a + b, 0) / 7;
                const trend = avgLastWeek > avgFirstWeek * 1.1 ? '📈 Naik' :
                    avgLastWeek < avgFirstWeek * 0.9 ? '📉 Turun' : '➡️ Stabil';

                // Update stats
                document.getElementById('todayCount').textContent = todayCount;
                document.getElementById('avgPerDay').textContent = avgPerDay;
                document.getElementById('maxPerDay').textContent = maxPerDay;
                ...
                gradientTotal.addColorStop(0, 'rgba(59, 130, 246, 0.3)');
                gradientTotal.addColorStop(1, 'rgba(59, 130, 246, 0.05)');

                const gradientSelesai = ctx.createLinearGradient(0, 0, 0, 400);
                gradientSelesai.addColorStop(0, 'rgba(16, 185, 129, 0.3)');
                gradientSelesai.addColorStop(1, 'rgba(16, 185, 129, 0.05)');

                const gradientDalam = ctx.createLinearGradient(0, 0, 0, 400);
                gradientDalam.addColorStop(0, 'rgba(245, 158, 11, 0.3)');
                gradientDalam.addColorStop(1, 'rgba(245, 158, 11, 0.05)');
            ...
            // Hapus function generateDailyData karena tidak perlu lagi
            // function generateDailyData(days) { ... } // HAPUS INI
            ...
            // Auto refresh chart data every 30 seconds (simulasi)
            setInterval(() => {
                // Simulate new data point
                if (dailyChart) {
                    const currentDays = document.querySelector('.tab-btn.active').dataset.period;
                    initChart(parseInt(currentDays));

                    // Add notification effect
                    const liveBadge = document.querySelector('.animate-pulse');
                    liveBadge.classList.remove('animate-pulse');
                    void liveBadge.offsetWidth; // Trigger reflow
